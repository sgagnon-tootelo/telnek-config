"""Session actions — password change via modal dialog (not inline sidebar form)."""
...
PASSWORD_BUTTON_KEY = "telnek_btn_change_password"
...
def _render_password_form(*, supabase: Any, t_fn: Callable[..., str], user_email: str) -> None:
    st.markdown(f"### {t_fn('password_change_title')}")
...
def _password_change_dialog(
    *, supabase: Any, t_fn: Callable[..., str], user_email: str
    ...
    _render_password_form(supabase=supabase, t_fn=t_fn, user_email=user_email)


def render_password_change_entry(
    *, supabase: Any, t_fn: Callable[..., str], user_email: str | None
) -> None:
    """Single sidebar button; form lives in a dialog to avoid widget-tree shifts."""
    if not can_change_password(user_email):
        st.caption(t_fn("password_change_dev_unavailable"))
        return

    if st.button(
        t_fn("password_change_open"),
        key=PASSWORD_BUTTON_KEY,
        use_container_width=True,
    ):
        _password_change_dialog(supabase=supabase, t_fn=t_fn, user_email=user_email)